...
The system SHALL store CMS authorization on the shared `User` record as a boolean `is_cms_user` column (Prisma field `isCmsUser`, default `false`). New users created via Google sign-in SHALL default to non-CMS (`is_cms_user = false`).

#### Scenario: New sign-up defaults to non-CMS
...
The NextAuth session SHALL include `user.isCmsUser` populated from the database `User` record during session resolution (database session strategy). The value SHALL reflect the current `is_cms_user` column when `auth()` is invoked on the server.
