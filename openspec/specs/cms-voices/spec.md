...
When the voice has `r2ObjectKey` set, the detail page SHALL render an audio player using a presigned URL. When `r2ObjectKey` is null, the page SHALL show metadata without a player (or a clear empty state) and SHALL NOT error.

#### Scenario: Preview available

- **WHEN** user views detail for a voice with `r2ObjectKey`
- **THEN** an HTML audio control plays the R2 object via presigned URL
