...
The system SHALL expose a tRPC `scriptGenerations.generate` mutation available to authenticated CMS users. The mutation SHALL accept `prompt` (non-empty string), `length` (one of `short`, `medium`, or `long`), `language` (a supported script language code), and optional `topicIds` (array of topic ID strings). It SHALL call Google Gemini server-side, persist a `ScriptGeneration` row for every attempt, and on success return `generationId`, `title`, and `content`.
