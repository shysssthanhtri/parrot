...
- [x] 6.1 Extend `speeches.getById` with thumbnail fields and resolved thumbnail URL
- [x] 6.2 Extend `speechPublications.list` and `getById` with `thumbnailUrl` from snapshot key

## 7. CMS UI

- [x] 7.1 Add Thumbnail card component under `src/app/(cms)/cms/speeches/[speechId]/_components/` (preview, status, regenerate, polling)
- [x] 7.2 Wire Thumbnail card into `speech-detail.tsx` / `speech-detail-client.tsx`
- [x] 7.3 Update Publishing card to use `getPublishReadiness` for **Publish** enablement and blocker copy
