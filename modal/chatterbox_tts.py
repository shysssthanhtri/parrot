...
#   --voice-key "system-voices/andy.wav"
#
# curl -X POST "https://<your-modal-endpoint>/generate" \
#   -H "Content-Type: application/json" \
#   -H "X-Api-Key: <your-api-key>" \
#   -d '{"prompt": "Hello from Chatterbox [chuckle].", "voice_key": "system-voices/andy.wav"}' \
#   --output output.wav

# R2 cloud bucket mount (read-only). voice_key values match Voice.r2ObjectKey
# (e.g. system-voices/andy.wav) — see prisma/seed.ts.
R2_BUCKET_NAME = os.environ.get("R2_BUCKET_NAME", "parrot")
...
app = modal.App("parrot-chatterbox-tts", image=image)
...
with image.imports():
    ...
    class TTSRequest(BaseModel):
        ...
        voice_key: str = Field(
            ...
            min_length=1,
            max_length=300,
            description="R2 object key for the voice sample (e.g. system-voices/andy.wav)",
        )
...
class Chatterbox:
    ...
    def serve(self):
        web_app = FastAPI(
            title="Parrot Chatterbox TTS API",
...
def test(
    ...
    voice_key: str = "system-voices/andy.wav",
